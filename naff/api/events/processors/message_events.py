...
class MessageEvents(EventMixinTemplate):
    ...
    async def _on_raw_message_create(self, event: "RawGatewayEvent") -> None:
        ...
        if not msg.guild_id and event.data.get("guild_id"):
            msg.guild_id = event.data["guild_id"]

        if not msg.author:
            # sometimes discord will only send an author ID, not the author. this catches that
            await self.cache.fetch_channel(to_snowflake(msg.channel_id)) if not msg.channel else msg.channel
            if msg.guild_id:
                await self.cache.fetch_guild(msg.guild_id) if not msg.guild else msg.guild
                await self.cache.fetch_member(msg.guild_id, msg.author_id)
            else:
                await self.cache.fetch_user(to_snowflake(msg.author_id))
